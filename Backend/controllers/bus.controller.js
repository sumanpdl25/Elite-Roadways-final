...
export const bookSeatHandler = async (req, res) => {
  ...
  try {
    ...
    const emailSent = await sendBookingConfirmationEmail(user.email, bookingDetails);
    
    if (!emailSent) {
      console.warn("Failed to send confirmation email, but booking was successful");
    }
